...
#setting paths
TRAIN_IMG_DIR = '/home/hlckwok2/research/unet_segmentation/Images' 
TRAIN_MASK_DIR = '/home/hlckwok2/research/unet_segmentation/Masks'          
INFERENCE_DIR = '/home/hlckwok2/research/Dataset'    
OUTPUT_DIR    = '/home/hlckwok2/research/segmentation2/Segmented_Results'      
RESCUED_DIR   = '/home/hlckwok2/research/segmentation2/Rescued_Results'      
ULTRA_DIR     = '/home/hlckwok2/research/segmentation2/Ultra_Rescued_Results'   
SKIPPED_DIR   = '/home/hlckwok2/research/segmentation2/Skipped_Images'         
...
def load_data(img_dir, mask_dir):
    ...
    log(f"{len(img_list)} images found, {len(mask_list)} masks found.")

    #create dictionary to read images
    mask_map = {}
    for m in mask_list:
    ...
    for f in img_list:
        ...
        clean_img_base = img_base.replace('mask_', '')
        ...
        if clean_img_base in mask_map:
            ...
            if img_cv is not None and mask_cv is not None:
                ...
                print(f"Failed to read {f} or mask.")
        else:
            print(f" No mask found for {f}")

    log(f"Paired {len(images)} out of {len(img_list)} images.")
...
#triple-pass segmentation
def run_segmentation(img_path, model, mode="first"):
    try:
        ...
        if contours:
            ...
            if radius > (w * 0.04) and not on_edge and solidity > sol_min:
                ...
                folders = {"first": OUTPUT_DIR, "second": RESCUED_DIR, "third": ULTRA_DIR}
                ...
                if mode != "first" and os.path.exists(img_path):
                    os.remove(img_path)
                return True
...
if __name__ == '__main__':
    ...
    log(f"Phase 1: {len(all_imgs)} images")
    ...
    for mode in ["second", "third"]:
        ...
        if not skipped: 
            ...
            continue
        log(f"Phase {mode}: {len(skipped)} images...")
        with ThreadPoolExecutor(max_workers=NUM_CORES) as ex:
