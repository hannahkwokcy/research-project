...
def plot_combined_history(pdf, h1, h2, metric_name):
    ...
    val_metrics = h1.history[f'val_{metric_name}'] + h2.history[f'val_{metric_name}']
    
    phase1_len = len(h1.history[metric_name])
    
    fig, ax = plt.subplots(figsize=(11.69, 8.27))
    plt.plot(train_metrics, label=f'Train {metric_name.capitalize()}', color='blue')
    plt.plot(val_metrics, label=f'Val {metric_name.capitalize()}', color='orange', linestyle='--')
