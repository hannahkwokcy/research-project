...
# set batch size and class map
batch_size = 64
class_mapping = ['Normal', 'Cataract']

# define data generators
train_datagen = ImageDataGenerator(
    # split data in 8:2 ratio for validation whilst training
...
# compile model and save relevant metrics
model.compile(
...
# define early stopping criteria for efficiency
early_stopping = EarlyStopping(
    monitor='val_loss', patience=patience_val, restore_best_weights=True)
model.summary()
...
history_p1 = model.fit(
    ...
    # use multiple cpus simultaneously for increased efficiency
    workers=16,
    use_multiprocessing=True,
    callbacks=early_stopping
)
# save the number of epochs ran
actual_epochs_p1 = len(history_p1.history['loss'])

# phase 2 fine-tuning
print("Phase 2: Unfreezing Top Layers...")
# unfreeze top layer for fine-tuning
...
def save_gradcam_plots(pdf, indices, title_prefix, images, labels, preds, model):
    for i in indices:
        ...
        heatmap = gradcam_plus_plus(img_tensor, model, 'conv5_block3_out')
...
#apply the new optimal threshold to  predictions
optimized_prediction = (
    prediction_probs >= optimal_threshold).astype(int).flatten()

#recalculate final metrics using the optimized predictions
opt_tn, opt_fp, opt_fn, opt_tp = confusion_matrix(
    y_true, optimized_prediction).ravel()
...
def plot_combined_history(pdf, h1, h2, metric_name):
    ...
    train_metrics = h1.history[metric_name] + h2.history[metric_name]
    val_metrics = h1.history[f'val_{metric_name}'] + \
        h2.history[f'val_{metric_name}']

    phase1_len = len(h1.history[metric_name])

    fig, ax = plt.subplots(figsize=(11.69, 8.27))
    plt.plot(train_metrics,
             label=f'Train {metric_name.capitalize()}', color='blue')
    plt.plot(
        val_metrics, label=f'Val {metric_name.capitalize()}', color='orange', linestyle='--')

    #mark the transition point
    plt.axvline(x=phase1_len - 1, color='red',
                linestyle=':', label='Fine-Tuning Starts')

    plt.title(f'{metric_name.capitalize()} Over Epochs (Two-Phase Training)')
...
'''
    ...
    #probabilities of predicting correct class
    probs_normal = prediction_probs[y_true == 0]
    probs_cataract = prediction_probs[y_true == 1]

    #plot overlapping histograms
    plt.hist(probs_normal, bins=50, alpha=0.6,
             color='blue', label='True Normal')
    plt.hist(probs_cataract, bins=50, alpha=0.6,
             color='red', label='True Cataract')
...
#combine the lists from phase 1 and 2
