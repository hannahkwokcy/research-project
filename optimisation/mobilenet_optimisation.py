...
class LRFinder(tf.keras.callbacks.Callback):
    def __init__(self, min_lr, max_lr, num_epochs, name="MobileNetV2"):
        super().__init__()
        self.min_lr = min_lr
        self.max_lr = max_lr
...
train_datagen = ImageDataGenerator(
    ...
    preprocessing_function=preprocess_input # Switched to MobileNetV2 preprocessing
)
...
lr_finder_p1 = LRFinder(min_lr=P1_MIN_LR, max_lr=P1_MAX_LR, num_epochs=P1_EPOCHS, name="MobileNetV2 Phase 1 (Frozen)")
model.fit(train_generator, epochs=P1_EPOCHS, validation_data=val_generator, callbacks=[lr_finder_p1])

lr_finder_p1.plot_lr_loss("mobilenet_phase1_lr.png")
...
# MobileNetV2 has 154 layers. Freezing up to the last few blocks (layer 100) 
...
lr_finder_p2 = LRFinder(min_lr=P2_MIN_LR, max_lr=P2_MAX_LR, num_epochs=P2_EPOCHS, name="MobileNetV2 Phase 2 (Fine-Tuning)")
model.fit(train_generator, epochs=P2_EPOCHS, validation_data=val_generator, callbacks=[lr_finder_p2])

lr_finder_p2.plot_lr_loss("mobilenet_phase2_lr.png")
