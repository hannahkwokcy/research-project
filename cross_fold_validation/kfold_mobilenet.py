...
def gradcam_plus_plus(img_array, model, last_conv_layer_name='out_relu'):
...
#store for aggregated statistics
tprs = []
aucs = []
...
for fold, (train_idx, val_idx) in enumerate(skf.split(train_df['filename'], train_df['class'])):
    ...
    model.compile(optimizer=Adam(learning_rate=lr_p2), 
    ...
    #store histories of current fold
    ...
    #collect other information for result generation
    ...
    #accumulate ensemble predictions on test Set
