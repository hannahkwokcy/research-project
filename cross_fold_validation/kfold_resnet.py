...
def gradcam_plus_plus(img_array, model, last_conv_layer_name='conv5_block3_out'):
...
for fold, (train_idx, val_idx) in enumerate(skf.split(train_df['filename'], train_df['class'])):
    ...
    if fold_auc > best_auc:
        ...
        print(f"*** New best fold (AUC: {best_auc:.4f}), saving entire model architecture + weights. ***")
...
print("\nGenerating Aggregate PDF Report with all Visuals...")
